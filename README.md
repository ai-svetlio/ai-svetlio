...
```
  ...
  Universal AI Agent Toolkit & Project Memory v2.1.1
```
...
- 🔒 **Iron Rules** — 22 правила за безопасна работа с AI агенти

---

## 📦 Инсталация

**Не е нужно да си програмист. Не е нужно да знаеш какво е терминал.**

Най-простият път е да оставиш агента да свърши инсталацията сам. Ти правиш две неща — той прави останалото.

### Стъпка 1 — това го правиш ти, веднъж

1. Свали приложението **Claude** за Windows или Mac: <https://claude.ai/download>
2. Влез в акаунта си.
3. Отвори таба **Code** и посочи папката, в която искаш да работиш (може и съвсем нова, празна).

> 💡 **Табът Code е мястото, където агентът може да чете и пише файлове по компютъра ти.**
> Това не е терминал — говориш с него на нормален език, както в обикновен чат.
> Ако си отваряш само чата, агентът не вижда файловете ти и нищо от долното няма да работи.

### Стъпка 2 — залепи това и натисни Enter

```text
Инсталирай ми AI_Svetlio в тази папка и ме преведи през първата сесия.

Прави го стъпка по стъпка и ми обяснявай какво става с прости думи:

1. Провери дали има Node.js (`node --version`) — трябва да е 18 или по-нова.
   Ако липсва или е стара, инсталирай я.
   На Windows: `winget install OpenJS.NodeJS.LTS`
   На Mac: `brew install node` (ако няма brew, дай ми линк да сваля Node.js сам и изчакай)
2. Инсталирай пакета: `npm install -g ai-svetlio@latest`
3. Провери, че командата се вижда: `svetlio --version`
   Ако не се разпознава — кажи ми да затворя и отворя наново прозореца.
4. Пусни `svetlio init` в тази папка.
5. Покажи ми какво е създадено и обясни с едно изречение за какво служи всяка папка.
6. Прочети CLAUDE.md и ми кажи с прости думи какви правила ще спазваш оттук нататък.

Важно: не приемай, че нещо е станало само защото не е дало грешка. Провери го по
изхода — `node --version`, `svetlio --version`, наличието на папка `.memory/` — и ми
покажи резултата. Ако нещо се провали, спри и ми обясни какво виждаш, вместо да
продължаваш нататък.
```
...
- Агентът ще ти иска **разрешение** за някои команди. Това е нормално и е добре — така виждаш какво прави.
- **Готово изглежда така:** `svetlio --version` изписва номер, а в папката ти има `.memory/` и `CLAUDE.md`.
- Ако Node.js се инсталира наново, **затвори и отвори прозореца** — иначе командата няма да се вижда.
...
Агентът ще прочете правилата и паметта на проекта и ще ти каже къде сте. Оттук нататък
всяка следваща сесия започва с `продължаваме`.

### Защо това не изяжда абонамента ти

Обичайното оплакване към работата с AI е, че излиза скъпо. В голямата си част това идва от
**преразказване**: всяка нова сесия започва от нула, човекът обяснява наново какво е проектът,
какво е правено вчера и защо, и чак тогава почва работа.

AI_Svetlio маха точно това. Състоянието живее в `.memory/` — агентът го прочита сам в началото
и го обновява в края. Нищо не се обяснява два пъти.

Разликата не е в модела, а в това дали помни.

### Ако предпочиташ на ръка

```bash
npm install -g ai-svetlio@latest
svetlio init
```
...
npm update -g ai-svetlio            # обновяване
svetlio upgrade                     # след обновяване — обнови правилата в проекта
```

### Ако нещо се обърка

| Симптом | Какво да направиш |
|---|---|
| `svetlio: command not found` / „не се разпознава" | Затвори и отвори наново прозореца. `npm -g` добавя пътя, но той се вижда чак в нов прозорец. |
| `node: command not found` | Node.js не е инсталирана или инсталацията не е довършена. Пусни промпта отгоре пак — агентът ще я хване. |
| На Mac: `EACCES` при `npm install -g` | Не слагай `sudo`. Кажи на агента да ти настрои npm префикс в домашната папка. |
| Агентът не вижда файловете ти | Работиш в обикновения чат, не в таба **Code**. Върни се на Стъпка 1. |
...
22 задължителни правила за AI агентите, които предотвратяват типични грешки:
...
└── .memory/
    ...
    │   └── MIGRATION_MAP.md
    │
    ├── backups/          ← Backups преди редакция (IRON RULE 5)
    │
    ├── plans/            ← Планове
    ├── architecture/     ← Архитектурни бележки и research
    └── scripts/          ← Помощни скриптове
```

> 🔒 **IRON RULE 15:** в корена на `.memory/` живеят САМО осемте стандартни файла.
> Всичко останало отива в поддиректория. Ad-hoc файл в корена = забранено.
...
`svetlio setup` приема и `--ide <cursor|claude-code|antigravity|all>`, ако искаш да настроиш само едно.
...
| `svetlio sync auto` | Включи авто-push при промяна |
| `svetlio sync remove` | Премахни връзката с hub |

### 🇧🇬 Команди на български

Всяка от тези работи еднакво с английския си вариант:

| Български | Английски |
|---|---|
| `svetlio уеб` | `svetlio web` |
| `svetlio обнови` | `svetlio upgrade` |
| `svetlio заявки` | `svetlio requests` |
| `svetlio синк настройка` | `svetlio sync init` |
| `svetlio синк изпрати` | `svetlio sync push` |
| `svetlio синк изтегли` | `svetlio sync pull` |
| `svetlio синк статус` | `svetlio sync status` |
| `svetlio синк авто` | `svetlio sync auto` |
| `svetlio синк премахни` | `svetlio sync remove` |
...
⭐ = препоръчано за начинаещи: приложението Claude, таб **Code**. Не иска терминал за ежедневната работа.

| Къде работиш | Rules файл | Статус |
|-----|------------|--------|
| **Приложението Claude → таб Code** ⭐ | `CLAUDE.md` | ✅ Пълна |
| Claude Code (терминал) | `CLAUDE.md` | ✅ Пълна |
| Cursor | `.cursorrules` | ✅ Пълна |
| Codex | `AGENTS.md` | ✅ Пълна |
...
### v2.1.1 (2026-09-07) — 🏷️ Само брандът

- 🏷️ **Credits:** авторството е **AI SvetLio** — брандът, не лично име. Без промени в кода.

### v2.1.0 (2026-09-05) — 🔒 Един източник на правилата

**Документацията и правилата вече не се преписват на ръка.**

- 🔒 **Един източник на правилата** (`src/rules.ts`) — `svetlio init`, `svetlio upgrade` и
  `documents/IRON_RULES.md` четат от едно място. Версията се чете от `package.json`.
  Преди това правилата живееха в три ръчни копия и документът изоставаше с 11 правила.
- 📄 **`documents/IRON_RULES.md` се ГЕНЕРИРА** (`npm run gen:rules`) и се регенерира автоматично
  преди всяко публикуване. Обясненията и примерите живеят в `documents/rules-examples/`.
- 🤖 **Codex поддръжка** — `svetlio init` и `svetlio upgrade` вече създават и `AGENTS.md`.
- 🛡️ **`svetlio upgrade` вече не трие лични настройки** — ако `~/.claude/CLAUDE.md` се различава
  от шаблона, прави backup в `~/.ai-svetlio/backups/` и показва пътя, преди да го презапише.
  Ако файлът е непроменен, изобщо не го пипа.
- 📦 **Нова секция „Инсталация"** — за човек, който не е програмист: приложението Claude → таб
  **Code**, и един готов промпт, който върши цялата инсталация.
- 📝 Поправени: банер v2.0.0 → v2.0.1 · „11 правила" → 22 · `sync` подкомандите и
  българските алиаси документирани · поддиректориите на `.memory/` (IRON RULE 15) · Credits.

### v2.0.1 (2026-04-18) — 🔒 IRON RULE 22 при upgrade

- 🔒 **`svetlio upgrade` налага защитата на паметта** — при обновяване на съществуващ проект
  добавя `.memory/` в `.gitignore`, ако липсва. Дотогава проекти, инициализирани преди
  правило 22, оставаха без защита и рискуваха да комитнат dev паметта си в публичен git.
...
- 🌐 **Hub Sync** — нов модул (`src/sync.ts`, 907 реда) — централизирана `.memory/` синхронизация чрез GitHub private repo
  - 6 команди: `svetlio sync init/push/pull/status/auto/remove` + БГ aliases (настройка, изпрати, изтегли, статус, авто, премахни)
...
Създаден от [AI SvetLio](https://github.com/ai-svetlio) — един автор, отворен код под MIT лиценз.
Ползвай го свободно, форквай го, променяй го.

**Софтуерът е свободен.** Плащат се настройката, хардуерът и работата с човека и агентите —
ако решиш, че искаш помощ за нещо по-сериозно.
