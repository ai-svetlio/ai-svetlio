// ============================================================================
// ЕДИНСТВЕНИЯТ ИЗВОР НА ПРАВИЛАТА (IRON RULE 21)
// ----------------------------------------------------------------------------
// Тук се дефинират IRON RULES и шаблонът на project CLAUDE.md. Всичко останало
// СЕ ГЕНЕРИРА от този файл:
//   - src/cli.ts    -> svetlio init / upgrade / setup
//   - src/modes.ts  -> режимите
//   - documents/IRON_RULES.md -> `npm run gen:rules`
//
// Не преписвай правила на второ място. Ако една истина стои на две места,
// второто се генерира от първото или не съществува.
// ============================================================================

import * as path from 'path';
import * as fs from 'fs-extra';

/** Версията се чете от package.json — никога не се пише на ръка. */
function readVersion(): string {
  try {
    return fs.readJsonSync(path.join(__dirname, '..', 'package.json')).version as string;
  } catch {
    return '0.0.0';
  }
}
...
/** Файловете с правила, които init/upgrade поддържат като IDENTICAL mirrors. */
export const RULE_FILES: string[] = [
...
/** Шаблонът на project CLAUDE.md — пълните IRON RULES. */
export function projectRules(version: string = VERSION): string {
